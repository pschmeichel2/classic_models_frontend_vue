  ...
  <v-dialog persistent v-model="show" max-width="500px" @keydown.esc="show = false">
    <v-card>
      <v-card-title class="blue darken-2 ">
        <v-row class="ma-1">
          <span class="text-h5 white--text">Delete Office</span>
        </v-row>
      </v-card-title>
      <p></p>
      <p></p>
      <v-card-text>
        <h3 class="font-weight-bold mb-3">Delete this Office?</h3>
        <span class="font-weight-bold mb-2">{{ city }}, {{ country }}</span>
      </v-card-text>
      <v-card-actions>
...
export default {
  ...
  methods: {
    ...
    open: function (office) {
      var vm = this;
      console.log('open', office.officeCode);
