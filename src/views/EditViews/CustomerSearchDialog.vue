            ...
            <v-card-text>
                ...
                <v-card>
                    <v-card-text>
                        <v-row >
                            <v-col cols="5">
                                <v-text-field label="Customer Name" dense v-model="customerName"
                                    maxlength="50" :rules="[rules.customerNameCounter]" counter
                                    @keyup.enter="find()" ref="customerNameRef"/>
                            </v-col>
                        </v-row><v-row >
                            <v-col cols="5">
                                <v-text-field label="Contact Last Name" dense v-model="contactLastName"
                                    maxlength="50" :rules="[rules.contactLastNameCounter]" counter
                                    @keyup.enter="find()" />
                            </v-col>
                            <v-col cols="5" >
                                <v-text-field label="Contact First Name" dense v-model="contactFirstName"
                                    maxlength="50" :rules="[rules.contactFirstNameCounter]" counter 
                                    @keyup.enter="find()"/>              
                            </v-col>
                        </v-row><v-row>
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-data-table :items="customers" :headers="customersHeaders" item-key="customerNumber" dense class="elevation-3"
                            :items-per-page="15" @click:row="handleClickCustomer" @dblclick:row="($event, {item})=>handleDblClickCustomer(item)"
                            height="250px" v-model="selected" ref="customerTableRef"
